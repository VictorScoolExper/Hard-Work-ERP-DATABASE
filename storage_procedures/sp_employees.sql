...
BEGIN
    ...
    INSERT INTO employees (user_id, created_by, edited_by, job_title, department, driver_license, start_date, wage_per_hour, image_name)
    VALUES (p_user_id, p_created_by_user, p_created_by_user, p_job_title, p_department, p_driver_license, p_start_date, p_wage_per_hour, p_image_name);
END

-- Get all Employees active or inactive
CREATE PROCEDURE get_employee_list()
BEGIN
    SELECT u.name, u.last_name, u.cell_number, u.role, u.birth_date, e.job_title, e.department, e.driver_license, e.start_date, e.end_date, e.wage_per_hour
    FROM users u
